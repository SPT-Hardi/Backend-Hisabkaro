    ...
    public class Users
    {
        ...
        public Result Add(Models.Developer.Subscriber.User value) 
        {
            using (TransactionScope scope = new TransactionScope())
            {
                using (HisabKaroDBDataContext context = new HisabKaroDBDataContext())
                {
                    ...
                    var newotp = context.SubOTPs.Where(x => x.DeviceToken == value.DeviceToken).SingleOrDefault();
                    if (newotp == null)
                    {
                        sotp.OTP = "456456";
        ...
        public Result VerifyOtp(Models.Developer.Subscriber.UserMobile value) 
        {
            using (HisabKaroDBDataContext context = new HisabKaroDBDataContext())
            {
                    ...
                    var qs = context.SubOTPs.Where(x => x.DeviceToken == value.DeviceToken).SingleOrDefault();
                    if (qs == null)
                    {
                    ...
                    var res = tokn.Add(qs.UId.ToString(), value.DeviceToken);
        ...
        public Result AddRole(Models.Developer.Subscriber.Role value,string UID) 
        {
            using (TransactionScope scope = new TransactionScope())
            {
                using (HisabKaroDBDataContext context = new HisabKaroDBDataContext())
