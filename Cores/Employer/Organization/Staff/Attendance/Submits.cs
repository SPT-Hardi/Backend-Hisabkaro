...
namespace HIsabKaro.Cores.Employer.Organization.Staff.Attendance
{
    ...
    {
        public Result Add(int URId,Models.Employer.Organization.Staff.Attendance.Submit value)
        {
            using (TransactionScope scope = new TransactionScope())
            {
                using (DBContext c = new DBContext())
                {
                    var qs = c.OrgStaffsAttendancesDailies.Where(x => x.ChekIN.Value.Day == DateTime.Now.Day && x.URId== value.URId).SingleOrDefault();
                    if (qs == null)
                    {
